...
class PurchaseRequest(models.Model):
    ...
    # add Fati MEEE
    @api.model
    def _get_default_requested_to(self):
        employee = self.env['hr.employee'].search([('user_id', '=', self.env.uid)], limit=1)
        if employee:
            gestionnaire = employee.parent_id.user_id
            return self.env['res.users'].browse(gestionnaire.id)
    ...
    titre = fields.Char('Titre', track_visibility='onchange')
    ...
    assigned_to = fields.Many2one(
        'res.users', 'Acheteur', track_visibility='onchange',
                                   default=_get_default_requested_to,
    ...
    date_approved = fields.Date(readonly=True, string='Date d\'approbation', copy=False)
    ...
    @api.multi
    def view_purchase_comparatif_line(self):
        action = self.env.ref(
            'purchase_request.purchase_request_line_form_action').read()[0]
        lines = self.mapped('line_ids')
        if lines:
            action['domain'] = [('id', 'in', lines.ids)]
            action['views'] = [(self.env.ref('purchase_request.purchase_request_line_tree_comparatif').id, 'tree')]
        #     action['res_id'] = lines.ids[0]
            return action
    @api.multi
    @api.depends(
    ...
    def button_to_approve(self):
        if self.assigned_to:
            partner_id = self._get_partner_id(self)
    ...
    def button_approved(self):
        return self.write({'state': 'approved', 'date_approved': fields.Date.today()})
    ...
    def to_approve_allowed_check(self):
        for rec in self:
            if not rec.to_approve_allowed:
                raise UserError(
                    _("Vous ne pouvez pas soumettre une demande vide ! (%s)") % rec.name)
